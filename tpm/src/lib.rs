...
pub fn execute_command(tpm: &mut TpmInstance, request: &[u8], response: &mut [u8]) {
    let mut offset = 0;
    let command_hdr = match unmarshal_command_header(request, &mut offset) {
        Ok(header) => header,
        Err(e) =>  {
            let response_hdr = ResponseHeader{tag: TpmCommandTag::NoSessions, size: 0, rc: e.rc};
    ...
    let response_hdr = match tpm.dispatch_command(&command_hdr, &request[COMMAND_HDR_SIZE..], &mut response[RESPONSE_HDR_SIZE..]) {
        Ok(size) => {
            ResponseHeader{tag: command_hdr.tag, size: size as u32, rc: TpmRc::Success}
        },
        Err(e) => {
            ResponseHeader{tag: command_hdr.tag, size: 0, rc: e.rc}
        }
    };
